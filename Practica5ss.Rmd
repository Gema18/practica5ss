...
No son discernibles todas las componentes sinusoidales de la señal puesto que obtenemos la misma representacion grafica de la resolución computacional y la resolucion fisica. 
...
yy = fft(xp);
t2 = (1:N-1) * Fm / N;
plot(t2,abs(yy), col = "darkblue", xlab="Frecuencias", ylab="Amplitud", main="Resolución Computacional", xlim = c(0,30))



```
CONCLUSIÓN:
Lo que estamos haciendo es aumentar la N con 0 padding y no aumentar tt, aumenta la resolución computacional.

La RF=1000/25 = 40Hz
La RC=1000/1000 = 1Hz
Como la física sigue siendo alta, no podremos discernir las frecuencias entre los picos representados.
...
#plot(t1,abs(y), col = "black", xlab="Frecuencias", ylab="Amplitud", main="Resolución Física", xlim=c(0, 30))
...
La RF=1000/100 = 10Hz
La RC=1000/100 = 10Hz
En este caso, lo que ocurre es que la resolución física es aceptable, pero la computacional no, por lo tanto ...
...
# Generamos el eje X para interpretar frecuencias
t1 = (1:t-1) * Fm / t;
plot(t1,abs(y), col = "red", xlab="Frecuencias", ylab="Amplitud", main="Resolución Física")
#plot(t1,abs(y), col = "red", xlab="Frecuencias", ylab="Amplitud", main="Resolución Física", xlim=c(0,30))
...
yy = fft(xp);
t2 = (1:N-1) * Fm / N;
plot(t2,abs(yy), col = "green", xlab="Frecuencias", ylab="Amplitud", main="Resolución Computacional")

#plot(t2,abs(yy), col = "green", xlab="Frecuencias", ylab="Amplitud", main="Resolución Computacional", xlim=c(0,30))

```

La RF = 10
La RC = 1
La RC y la RF están los suficientemente equilibradas como para poder distinguir todas las frecuencias y 
...
# Generamos el eje X para interpretar frecuencias
t1 = (1:t-1) * Fm / t;
plot(t1,abs(xy), col = "red", xlab="Frecuencias", ylab="Amplitud", main="Resolución Física")

plot(t1,abs(xy), col = "red", xlab="Frecuencias", ylab="Amplitud", main="Resolución Física", xlim=c(0, 30))
...
La RF = 4
La RC = 4
Lo que ocurre en este apartado es lo que se conoce como spectral leakage. Ocurrirá si alguna de las com...
...
```{r}
#Además, este código nos sirve para conocer la representación gráfica de la ventana Hamming
rm(list=ls())
N1 = 40
n1 = c(1:N1-1)
dim(n1)= c(1,N1)
...
#subplot(p1, p2, p3,p4, nrows=2)
#Vemos que lo que ocurre es que se aplanan los picos de las sinusoides. Esto ocurre porque la ventana
```
...
#subplot(p1, p2, p3,p4, nrows=2)
#En general, la ventana Hanning es satisfactoria en el 95% de los casos. Tiene buena resolución de frecuencia
#Ventana Blackman
rm(list=ls())
N1 = 40
n1 = c(1:N1-1)
...
#subplot(p1, p2, p3,p4, nrows=2)
#La Blackman-Harris es similar a las ventanas Hamming y Hann. El espectro resultante tiene un pico ampli
```
...
No hay enventanado perfecto. Cada uno perjudica la representación en algún aspecto. De
hecho, existe una tabla que, dependiendo de la señal a representar, sugiere el mejor enventanado.
...
plot(dft, xlim=c(0,2048), ylim=c(0,250000), xlab="N", ylab="Carga computacional", col="black")
par(new=T, col = "red")
plot(fft, xlim=c(0,2048), ylim=c(0,250000), xlab="N", ylab="Carga computacional")

#Vemo cómo la función logarítmica tiene un crecimiento mucho más suave, es menos pronunciado. Mientras ........
```
